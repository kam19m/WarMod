...
new Handle:g_cleaner_timer = INVALID_HANDLE;
new Handle:g_h_hud_timer = INVALID_HANDLE;
new Handle:g_hCountdownTimer = INVALID_HANDLE;

/* stats */
new bool:g_log_warmod_dir = false;
new String:g_log_filename[256];
...
new bool:g_bPendingEWar = false;
/* Player score preservation across halftime */
new g_saved_kills[MAXPLAYERS + 1];
new g_saved_deaths[MAXPLAYERS + 1];
new bool:g_should_restore_player_scores = false;
...
new bool:g_bShowStatusMessage = false;
new Handle:g_hHideStatusTimer = INVALID_HANDLE;
/* modes */
...
public OnPluginStart()
{
	...
	AutoExecConfig(true, "warmod_colors");
	...
	g_h_save_file_dir = CreateConVar("wm_save_dir", "demos", "Directory to store SourceTV demos and WarMod logs");
	...
	HookUserMessage(GetUserMessageId("RadioText"), Hook_RadioText, true);
...
public OnMapStart()
{
	g_cleaner_timer = INVALID_HANDLE;
	g_h_hud_timer = INVALID_HANDLE;
	...
	if (g_h_hud_timer != INVALID_HANDLE)
	{
		KillTimer(g_h_hud_timer);
	}
...
ResetMatch(bool:silent)
{
	...
	if (!silent)
	{
		WM_CPrintToChatAll("Match Reset - Warmup Active");
	}
}
...
ResetHalf(bool:silent)
{
	if (g_match)
	{
		CallForwardSafe(g_f_on_reset_half);
		if (GetConVarBool(g_h_stats_enabled))
		{
	...
	if (!silent)
	{
		for (new x = 1; x <= 3; x++)
		{
			WM_CPrintToChatAll("%t", "Half Reset");
		}
...
public Action:ReadyToggle(client, args)
{
	...
	if (!IsReadyEnabled(client, true))
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Ready System Enabled");
		}
		else
		{
			WM_PrintToServer("%T", "Ready System Enabled", LANG_SERVER);
		}	
		CheckReady();
	}
	else
	{
		...
		if (client != 0)
		{
			WM_PrintToConsole(client, "%t", "Ready System Disabled");
		}
		else
		{
			WM_PrintToServer("%T", "Ready System Disabled", LANG_SERVER);
		}
	}
...
public Action:ActiveToggle(client, args)
{
	...
	if (GetConVarBool(g_h_active))
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Set Inactive");
		}
		else
		{
			WM_PrintToServer("%T", "Set Inactive", LANG_SERVER);
		}
	}
	else
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Set Active");
		}
		else
		{
			WM_PrintToServer("%T", "Set Active", LANG_SERVER);
		}
	}
...
public Action:ChangeMinReady(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Set Minready", minready);
		}
		else
		{
			WM_PrintToServer("%T", "Set Minready", LANG_SERVER, minready);
		}
	...
	else
	{
		if (client != 0)
		{
			WM_CPrintToChat(client, "wm_min_ready = %d", GetConVarInt(g_h_min_ready));
		}
		else
		{
			WM_PrintToServer("wm_min_ready = %d", GetConVarInt(g_h_min_ready));
		}
	}
...
public Action:ChangeMaxRounds(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Set Maxrounds", maxrounds);
		}
		else
		{
			WM_PrintToServer("%T", "Set Maxrounds", LANG_SERVER, maxrounds);
		}
	...
	else
	{
		if (client != 0)
		{
			WM_CPrintToChat(client, "wm_max_rounds = %d", GetConVarInt(g_h_max_rounds));
		}
		else
		{
			WM_PrintToServer("wm_max_rounds = %d", GetConVarInt(g_h_max_rounds));
		}
	}
...
public Action:ChangePassword(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Set Password", new_password);
		}
		else
		{
			WM_PrintToServer("%T", "Set Password", LANG_SERVER, new_password);
		}
	...
	else
	{
		...
		new Handle:hSvPassword = FindConVar("sv_password");
		if (hSvPassword != INVALID_HANDLE)
		{
			GetConVarString(hSvPassword, passwd, sizeof(passwd));
		}
		if (client != 0)
		{
			WM_CPrintToChat(client, "sv_password = '%s'", passwd);
		}
		else
		{
			WM_PrintToServer("sv_password = '%s'", passwd);
		}
	}
...
ReadyUp(client)
{
	...
	if (client == 0)
	{
		WM_PrintToServer("%T", "Console Ready", LANG_SERVER);
		return;
	}
	...
	if (g_player_list[client] != PLAYER_READY)
	{
		...
		{
			WM_CPrintToChat(client, "%t", "Not on Team");
		}
	}
	else
	{
		WM_CPrintToChat(client, "%t", "Already Ready");
	}
}
...
ReadyDown(client)
{
	...
	if (client == 0)
	{
		WM_PrintToServer("%T", "Console Not Ready", LANG_SERVER);
		return;
	}
	...
	{
		WM_CPrintToChat(client, "%t", "Already Not Ready");
	}
}
...
public Action:ForceAllReady(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		{
			WM_PrintToConsole(client, "%T", "Forced Ready", LANG_SERVER);
		}
	...
	else
	{
		...
		{
			WM_PrintToConsole(client, "%T", "Ready System Disabled2", LANG_SERVER);
		}
	}
...
public Action:ForceAllUnready(client, args)
{
	...
	if (g_ready_enabled)
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Forced Not Ready");
		}
		else
		{
			WM_PrintToServer("%T", "Forced Not Ready", LANG_SERVER);
		}
	...
	else
	{
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Ready System Disabled2");
		}
		else
		{
			WM_PrintToServer("%T", "Ready System Disabled2", LANG_SERVER);
		}
	}
...
public Action:ReadyOn(client, args)
{
	...
	if (client != 0)
	{
		WM_PrintToConsole(client, "%t", "Ready System Enabled");
	}
	else
	{
		WM_PrintToServer("%T", "Ready System Enabled", LANG_SERVER);
...
public Action:ConsoleScore(client, args)
{
	if (g_match)
	{
		if (g_live)
		{
			if (client != 0)
			{
				WM_PrintToConsole(client, "%t:", "Match Is Live");
			}
			else
			{
				WM_PrintToServer("%T:", "Match Is Live", LANG_SERVER);
			}
		}
		WM_PrintToConsole(client, "%s: [%d] %s: [%d] MR%d", g_t_name, GetTScore(), g_ct_name, GetCTScore(), GetConVarInt(g_h_max_rounds));
		if (g_overtime)
		{
			WM_PrintToConsole(client, "%t (%d): [%d] %s: [%d] MR%d", "Score Overtime", g_overtime_count + 1, g_t_name, GetTOTScore(), g_ct_name, GetCTOTScore(), GetConVarInt(g_h_overtime_mr));
		}
	}
	else
	{
		if (client != 0)
		{
			WM_PrintToConsole(client, "%t", "Match Not In Progress");
		}
		else
		{
			WM_PrintToServer("%T", "Match Not In Progress", LANG_SERVER);
		}
	}
...
ShowScore(client)
{
	...
	{
		WM_CPrintToChat(client, "%t", "Match Not In Progress");
	}
...
DisplayScore(client, msgindex, bool:priv)
{
	...
	if (msgindex == 0) // standard play score
	{
		...
		if (priv)
		{
			WM_CPrintToChat(client, "%s", score_msg);
		}
		else
		{
			WM_CPrintToChatAll("%s", score_msg);
		}
	}
	else if (msgindex == 1) // overtime play score
	{
		...
		{
			WM_CPrintToChatAll("%t%s", "Score Overtime", score_msg);
		}
	}
	else if (msgindex == 2) // overall play score
	{
		...
		if (priv)
		{
			WM_CPrintToChat(client, "%t%s", "Score Overall", score_msg);
		}
		else
		{
			WM_CPrintToChatAll("%t%s", "Score Overall", score_msg);
		}
	}
}
...
public GetScoreMsg(client, String:result[], maxlen, t_score, ct_score)
{
	...
	decl String:color_t[32];
	decl String:color_ct[32];
	GetConVarString(g_h_text_color, t_color, sizeof(t_color));
	GetConVarString(g_h_color_t, color_t, sizeof(color_t));
	GetConVarString(g_h_color_ct, color_ct, sizeof(color_ct));
	
	decl String:colored_team[128];
	
	if (t_score > ct_score)
	{
		Format(colored_team, sizeof(colored_team), "%s%s%s", color_t, g_t_name, t_color);
		Format(result, maxlen, "%s%t %s%d%s-%s%d", t_color, "T Winning", colored_team, color_t, t_score, t_color, color_ct, ct_score);
	}
	else if (t_score == ct_score)
	{
		Format(result, maxlen, "%s%t %s%d%s-%s%d", t_color, "Tied", color_t, t_score, t_color, color_ct, ct_score);
	}
	else
	{
		Format(colored_team, sizeof(colored_team), "%s%s%s", color_ct, g_ct_name, t_color);
		Format(result, maxlen, "%s%t %s%d%s-%s%d", t_color, "CT Winning", colored_team, color_ct, ct_score, t_color, color_t, t_score);
	}
}
...
{
	CreateTimer(1.0, Timer_SyncScoreboard, _, TIMER_FLAG_NO_MAPCHANGE);
	round_end = false;
	...
	if (GetConVarBool(g_h_stats_enabled))
	{
		new Handle:hFreezeTime = FindConVar("mp_freezetime");
		if (hFreezeTime != INVALID_HANDLE)
		{
			Log2Game("\"round_start\" (freezetime \"%d\")", GetConVarInt(hFreezeTime));
		}
	}
	...
	if (!g_live)
	{
		...
		{
			g_cleaner_timer = CreateTimer(3.0, Timer_CleanWeapons, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
		}
		
		for (new iW = 1; iW <= MaxClients; iW++)
		{
			if (IsClientInGame(iW) && IsPlayerAlive(iW))
			{
				new iWeapon = GetPlayerWeaponSlot(iW, 4);
				if (iWeapon != -1)
				{
					RemovePlayerItem(iW, iWeapon);
					AcceptEntityInput(iWeapon, "Kill");
				}
				
				if (GetEntProp(iW, Prop_Send, "m_bHasDefuser"))
				{
					SetEntProp(iW, Prop_Send, "m_bHasDefuser", 0);
				}
				
				SetEntData(iW, g_i_account, 16000);
			}
		}
	...
	if (!g_t_score) g_t_score = true;
	
	if (g_t_knife)
	{
		for (new iK = 1; iK <= MaxClients; iK++)
		{
			if (IsClientInGame(iK) && GetClientTeam(iK) > 1)
			{
				SetEntData(iK, g_i_account, 0);
				CS_StripButKnife(iK);
				if (GetConVarBool(g_h_knife_hegrenade)) GivePlayerItem(iK, "weapon_hegrenade");
				if (GetConVarInt(g_h_knife_flashbang) >= 1)
				{
					GivePlayerItem(iK, "weapon_flashbang");
					if (GetConVarInt(g_h_knife_flashbang) >= 2) GivePlayerItem(iK, "weapon_flashbang");
				}
				if (GetConVarBool(g_h_knife_smokegrenade)) GivePlayerItem(iK, "weapon_smokegrenade");
			}
		}
	}
	
	// Money display logic (Now correctly inside the function)
	if (g_match && g_live && !g_t_knife && g_t_money && GetConVarBool(g_h_round_money) && g_i_account != -1)
	{
		new the_money[MAXPLAYERS + 1];
		new num_players = 0;
		
		for (new iP = 1; iP <= MaxClients; iP++)
		{
			if (IsClientInGame(iP) && !IsFakeClient(iP) && GetClientTeam(iP) > 1)
			{
				the_money[num_players] = iP;
				num_players++;
			}
		}
		...
		new String:m_player_name[32];
		new String:m_player_money[10];
		
		decl String:t_color[32];
		decl String:color_t[32];
		decl String:color_ct[32];
		decl String:color_money[32];
		GetConVarString(g_h_text_color, t_color, sizeof(t_color));
		GetConVarString(g_h_color_t, color_t, sizeof(color_t));
		GetConVarString(g_h_color_ct, color_ct, sizeof(color_ct));
		GetConVarString(g_h_color_money, color_money, sizeof(color_money));

		for (new iR = 1; iR <= MaxClients; iR++)
		{
			if (!IsClientInGame(iR) || IsFakeClient(iR)) continue;
			
			for (new x = 0; x < num_players; x++)
			{
				if (GetClientTeam(iR) == GetClientTeam(the_money[x]))
				{
					new client_team = GetClientTeam(the_money[x]);
					decl String:team_color[32];
					
					if (client_team == 2)
					{
						Format(team_color, sizeof(team_color), "%s", color_t);
					}
					else if (client_team == 3)
					{
						Format(team_color, sizeof(team_color), "%s", color_ct);
					}
					else
					{
						Format(team_color, sizeof(team_color), "%s", t_color);
					}

					GetClientName(the_money[x], m_player_name, sizeof(m_player_name));
					IntToMoney(GetEntData(the_money[x], g_i_account), m_player_money, sizeof(m_player_money));
					
					CPrintToChat(iR, "%s$%s %s> %s%s", color_money, m_player_money, t_color, team_color, m_player_name);
				}
			}
		}
	}
...
stock ShowTeamMoney(client)
{
	...
	CPrintToChat(client, "%s--------", t_color);
	for (new x = 0; x < num_players; x++)
	{
		if (!IsClientInGame(the_money[x])) continue;
		...
		if (IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == GetClientTeam(the_money[x]))
		{
			...
			CPrintToChat(client, "%s$%s %s%s> %s%s", color_money, player_money, t_color, has_weapon, team_color, player_name);
		}
	}
}
	...
	if (!g_live || g_t_knife)
	{
		for (new i = 1; i <= MaxClients; i++)
		{
			if (IsClientInGame(i) && IsPlayerAlive(i))
			{
				...
				new weapon = -1;
				while ((weapon = GetPlayerWeaponSlot(i, 3)) != -1)
				{
...
public Event_Player_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		CreateTimer(0.1, Timer_StripC4, client);
	}
}

public Action:Timer_StripC4(Handle:timer, any:client)
{
	if (!g_live && IsClientInGame(client) && IsPlayerAlive(client) && GetClientTeam(client) == 2)
	{
		new iWeapon = GetPlayerWeaponSlot(client, 4);
		if (iWeapon != -1)
		{
...
public Event_Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
				...
				if (GetNumAlive(GetClientTeam(victim)) > 0)
				{
					...
					for (new i = 1; i <= MaxClients; i++)
					{
						if (i != victim && IsClientInGame(i) && !IsFakeClient(i) && !IsPlayerAlive(i) && GetClientTeam(i) == GetClientTeam(victim))
						{
							...
							if (target > 0 && target <= MaxClients && IsClientInGame(target) && IsClientObserver(target))
							{
	...
	if (!g_live)
	{
		CreateTimer(0.0, RemoveRagdoll, victim, TIMER_FLAG_NO_MAPCHANGE);
	}
	else if (GetConVarBool(g_h_body_remove) && GetNumAlive(GetClientTeam(victim)) > 0)
	{
...
public Action:Timer_StartShowingStatus(Handle:timer)
{
    g_bShowStatusMessage = true;
    if (g_hHideStatusTimer != INVALID_HANDLE) KillTimer(g_hHideStatusTimer);
    g_hHideStatusTimer = CreateTimer(3.0, Timer_StopShowingStatus, _, TIMER_FLAG_NO_MAPCHANGE);
    return Plugin_Stop;
}

public Action:Timer_StopShowingStatus(Handle:timer)
{
    g_bShowStatusMessage = false;
    g_hHideStatusTimer = INVALID_HANDLE;
    return Plugin_Stop;
}
...
public Event_Bomb_Exploded(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client > 0 && !GetConVarBool(g_h_bomb_frags) && !round_end) // must not be after round end as they no longer get frags
	{
...
public Event_Item_Pickup(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client > 0 && IsClientInGame(client))
	{
		...
		if (!g_live || g_t_knife)
		{
			if (StrContains(item, "hegren", false) != -1 || 
				StrContains(item, "flash", false) != -1 || 
				StrContains(item, "smoke", false) != -1)
			{
				new weapon = -1;
			...
			else if (StrContains(item, "defuser", false) != -1)
			{
	...
	if (GetConVarBool(g_h_stats_enabled))
	{
		new String:log_string[256];
		CS_GetAdvLogString(client, log_string, sizeof(log_string));
		new String:item_log[64];
		GetEventString(event, "item", item_log, sizeof(item_log));
		Log2Game("\"item_pickup\" (player \"%s\") (item \"%s\")", log_string, item_log);
	}
}
	...
	if (GetConVarInt(g_h_score_mode) == 1)
	{
		if (!g_overtime)
		{
			if (GetScore() == GetConVarInt(g_h_max_rounds)) // half time
			{
				...
				CallForwardSafe(g_f_on_half_time);
				if (GetConVarBool(g_h_stats_enabled))
				{
				...
				{
					WM_CPrintToChatAll("%t", "Half Time");
				}
				else
				{
					WM_CPrintToChatAll("%t", "Half Time Auto Swap");
			...
			else if (GetTScore() == GetConVarInt(g_h_max_rounds) && GetCTScore() == GetConVarInt(g_h_max_rounds)) // complete draw
			{
				...
				{ // max rounds overtime
					...
					DisplayScore(0, 0, false);
					WM_CPrintToChatAll("%t", "Over Time", GetConVarInt(g_h_overtime_mr));
					g_live = false;
					g_t_money = false;
					g_overtime = true;
					g_overtime_mode = 1;
				...
				else if (GetConVarInt(g_h_overtime) == 2) // sudden death overtime
				{
					...
					DisplayScore(0, 0, false);
					WM_CPrintToChatAll("%t", "Over Time Sudden Death");
					g_live = false;
					g_t_money = false;
					g_overtime = true;
					g_overtime_mode = 2;
				...
				{
					CallForwardSafe(g_f_on_end_match);
					if (GetConVarBool(g_h_stats_enabled))
					{
					...
					WM_CPrintToChatAll("%t", "Full Time");
					...
					CreateTimer(3.0, Timer_DelayedEndMatch, _);
				}
			}
			...
			else if (!g_playing_out && GetTScore() == GetConVarInt(g_h_max_rounds) + 1 || GetCTScore() == GetConVarInt(g_h_max_rounds) + 1) // full time
			{
				...
				else
				{
					...
					g_playing_out = true;
					WM_CPrintToChatAll("%t", "Playing Out Notice", (GetConVarInt(g_h_max_rounds) * 2));
				}
			}
			else
			{
		...
		else
		{
			...
			else if (GetTOTScore() == GetConVarInt(g_h_overtime_mr) && GetCTOTScore() == GetConVarInt(g_h_overtime_mr)) // complete draw
			{
				...
				{ // max rounds overtime
					...
					g_live = false;
					g_t_money = false;
					if (g_overtime_count < 255) g_overtime_count++;
					g_first_half = true;
					SetAllCancelled(false);
					ReadyChangeAll(0, false, true);
...
{
	if (!IsClientInGame(elem1) || !IsClientInGame(elem2)) return 0;
...
ReadyServ(client, bool:ready, bool:silent, bool:show, bool:priv)
{
	...
	if (ready)
	{
		...
		if (!silent)
		{
			WM_CPrintToChat(client, "%t", "Ready");
		}
	}
	else
	{
		...
		if (!silent)
		{
			WM_CPrintToChat(client, "%t", "Not Ready");
		}
	}
...
LiveOn3(bool:e_war)
{
	g_bPendingEWar = e_war;
	g_iCountdown = 5;
	
	if (g_hCountdownTimer != INVALID_HANDLE) KillTimer(g_hCountdownTimer);
...
ExecuteLiveOn3(bool:e_war)
{
	...
	if (!g_match)
	{
		...
		new String:full_dir_path[256];
		Format(full_dir_path, sizeof(full_dir_path), "%s", save_dir);
		
		if (!DirExists(full_dir_path))
		{
			CreateDirectory(full_dir_path, 511);
		}
...
stock LiveOn3Override()
{
	...
	if (KvJumpToKey(kv, "live_first_restart"))
	{
		...
		CreateTimer(0.1, RestartRound, 1);
		new Handle:datapack;
		CreateDataTimer(0.2, PrintToChatDelayed, datapack);
	...
	{
		KvGetString(kv, "text", text, sizeof(text));
		CreateTimer(1.2, RestartRound, 1);
		new Handle:datapack;
		CreateDataTimer(1.3, PrintToChatDelayed, datapack);
		WritePackString(datapack, text);
		KvGoBack(kv);
	}
	...
	{
		KvGetString(kv, "text", text, sizeof(text));
		CreateTimer(2.3, RestartRound, 1);
		new Handle:datapack;
		CreateDataTimer(2.4, PrintToChatDelayed, datapack);
		WritePackString(datapack, text);
		KvGoBack(kv);
	}
	if (KvJumpToKey(kv, "live_finished"))
	{
		...
		for (new i = 1; i <= 5; i++)
		{
			...
			{
				new Handle:datapack;
				CreateDataTimer(3.5, PrintToChatDelayed, datapack);
				WritePackString(datapack, text);
			}
		}
	}
...
public Action:ChooseTeam(client, args)
{
	...
	{
		WM_CPrintToChat(client, "%t", "Change Teams Midgame");
		return Plugin_Stop;
	}
	
	new max_players = GetConVarInt(g_h_max_players);
	if (g_match && max_players != 0 && GetClientTeam(client) <= 1 && CS_GetPlayingCount() >= max_players)
	{
		WM_CPrintToChat(client, "%t", "Maximum Players");
...
public Action:RestrictBuy(client, args)
{
	...
	{
		WM_CPrintToChat(client, "%t", "Nightvision Blocked");
		return Plugin_Handled;
	}
	
	new String:the_weapon[32];
	Format(the_weapon, sizeof(the_weapon), "%s", arg);
	ReplaceString(the_weapon, sizeof(the_weapon), "\"", "");
	...
	if (!g_live || g_t_knife)
	{
		if (StrContains(the_weapon, "hegren", false) != -1 || 
			StrContains(the_weapon, "flash", false) != -1 || 
			StrContains(the_weapon, "smokegrenade", false) != -1)
		{
			WM_CPrintToChat(client, "This item is not available on warmup");
			return Plugin_Handled;
		}
	}
...
public Action:ReadyList(client, args)
{
	...
	WM_CReplyToCommand(client, "%T:", "Ready System", LANG_SERVER);
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) > 1)
		{
...
{ 
	...
	if (client == 0)
	{
		WM_PrintToServer("%T", "Half Reset", LANG_SERVER);
	}
...
public Action:CancelMatch(client, args)
{
	...
	if (client == 0)
	{
		WM_PrintToServer("%T", "Match Reset", LANG_SERVER);
	}
...
public Action:CancelKnife(client, args)
{
	...
	if (g_t_knife)
	{
		...
		for (new x = 1; x <= 3; x++)
		{
			WM_CPrintToChatAll("%t", "Knife Round Cancelled");
		}
		if (client == 0)
		{
			WM_PrintToServer("%T", "Knife Round Cancelled", LANG_SERVER);
		}
	}
	else
	{
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Knife Round Inactive");
		}
		else
		{
			WM_PrintToServer("%T", "Knife Round Inactive", LANG_SERVER);
		}
	}
...
ReadyChangeAll(client, bool:up, bool:silent)
{
	...
	if (!silent && client > 0 && IsClientInGame(client))
    {
        ShowInfo(client, true, true, 0);
    }
}
...
IsLive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				WM_CPrintToChat(client, "%t", "Match Is Live");
			}
			else
			{
				WM_PrintToServer("%T", "Match Is Live", LANG_SERVER);
...
IsActive(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			if (client != 0)
			{
				WM_CPrintToChat(client, "%t", "WarMod Inactive");
			}
			else
			{
				WM_PrintToServer("%T", "WarMod Inactive", LANG_SERVER);
			}
		}
	}
	return false;
...
IsAdminCmd(client, bool:silent)
{
	...
	else
	{
		if (!silent)
		{
			WM_CPrintToChat(client, "%t", "WarMod Rcon Only");
		}
	}
	return false;
}
...
public Action:ChangeT(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Change T Name", name);
		}
		else
		{
			WM_PrintToServer("%T", "Change T Name", LANG_SERVER, name);
		}
		CheckReady();
	...
	else
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "wm_t = %s", name);
		}
		else
		{
			WM_PrintToServer("wm_t = %s", name);
		}
	}
...
public Action:ChangeCT(client, args)
{
	...
	if (GetCmdArgs() > 0)
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "%t", "Change CT Name", name);
		}
		else
		{
			WM_PrintToServer("%T", "Change CT Name", LANG_SERVER, name);
		}
		CheckReady();
	...
	else
	{
		...
		if (client != 0)
		{
			WM_CPrintToChat(client, "wm_ct = %s", name);
		}
		else
		{
			WM_PrintToServer("wm_ct = %s", name);
		}
	}
...
public Action:SayChat(client, args)
{
	...
	else
	{
		...
		if (text[start_index] == '@' && GetConVarBool(g_h_global_chat))
		{
			...
			{
				WM_CPrintToChat(client, "%t", "No Permission");
			}
			return Plugin_Handled;
		}
		...
		else if (StrEqual(command, "info", false) || StrEqual(command, "i", false))
		{
			...
			{
				WM_CPrintToChat(client, "%t", "ShowInfo Disabled");
			}
		}
	...
	if ((GetConVarBool(g_h_req_names) && g_ready_enabled && !g_live && (StrEqual(g_t_name, DEFAULT_T_NAME, false) || StrEqual(g_ct_name, DEFAULT_CT_NAME, false))) && (!GetConVarBool(g_h_auto_knife) || g_t_had_knife))
	{
		...
		if (num_ready >= GetConVarInt(g_h_min_ready))
		{
			for (new i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && !IsFakeClient(i))
				{
					WM_CPrintToChat(i, "%t", "Names Required");
				}
			}
		}
	}
...
public Action:Timer_SafeStrip(Handle:timer, any:client)
{
...
public Action:ShowPluginInfo(Handle:timer, any:client)
{
	if (client != 0 && IsClientConnected(client) && IsClientInGame(client))
	{
		...
		PrintToConsole(client, "This server is running Woobbie x GameTech / WarMod %s Server Plugin", WM_VERSION);
		PrintToConsole(client, "");
		PrintToConsole(client, "Created by Woobbie x GameTech");
		PrintToConsole(client, "");
...
{
	if (!g_live && StrEqual(classname, "item_defuser"))
	{
		AcceptEntityInput(entity, "Kill");
	}
}

public Action:Timer_CleanWeapons(Handle:timer)
{
	if (g_live)
	{
		g_cleaner_timer = INVALID_HANDLE;
	...
	for (new ent = MaxClients + 1; ent <= max_ents; ent++)
	{
		if (IsValidEntity(ent) && IsValidEdict(ent))
		{
			GetEdictClassname(ent, clsname, sizeof(clsname));
			
			if (StrContains(clsname, "weapon_") == 0 || StrContains(clsname, "item_") == 0)
			{
				if (HasEntProp(ent, Prop_Send, "m_hOwnerEntity") && GetEntPropEnt(ent, Prop_Send, "m_hOwnerEntity") == -1)
				{
					if (HasEntProp(ent, Prop_Data, "m_iHammerID") && GetEntProp(ent, Prop_Data, "m_iHammerID") > 0)
					{
						continue;
					}
...
public Action:Timer_WarmupHUD(Handle:timer)
{
    if (g_live)
    {
        return Plugin_Continue;
    ...
    new total_players = 0;
    new min_ready = GetConVarInt(g_h_min_ready);

    for (new i_player = 1; i_player <= MaxClients; i_player++)
    {
        if (IsClientInGame(i_player) && !IsFakeClient(i_player) && GetClientTeam(i_player) >= 1)
        {
            total_players++;
        }
    }

    if (total_players < min_ready)
    {
        new needed = min_ready - total_players;
        PrintHintTextToAll("Not enough players : %d", needed);
	}
...
public Action:Timer_Countdown(Handle:timer)
{
	if (g_iCountdown > 0)
	{
		for (new i_player = 1; i_player <= MaxClients; i_player++)
		{
			if (IsClientInGame(i_player) && !IsFakeClient(i_player))
			{
				new Handle:panel = CreatePanel();
				decl String:buffer[64];
				Format(buffer, sizeof(buffer), "Starting in: %d", g_iCountdown);
				SetPanelTitle(panel, buffer);
				
				SendPanelToClient(panel, i_player, Handler_DoNothing, 1);
				CloseHandle(panel);
			}
		}
...
stock SyncScoreboard()
{
    new t_score = 0;
    new ct_score = 0;

    if (g_first_half || g_live)
    {
        t_score = GetTTotalScore();
        ct_score = GetCTTotalScore();
    }
    ...
    while ((ent = FindEntityByClassname(ent, "cs_team_manager")) != -1)
    {
        if (IsValidEntity(ent))
        {
            ...
            if (team == 2)
            {
                if (HasEntProp(ent, Prop_Send, "m_iScore"))
                {
                    SetEntProp(ent, Prop_Send, "m_iScore", t_score);
                }
            }
            else if (team == 3)
            {
                if (HasEntProp(ent, Prop_Send, "m_iScore"))
                {
                    SetEntProp(ent, Prop_Send, "m_iScore", ct_score);
                }
            }
        }
    }
...
public Action:Timer_SyncScoreboard(Handle:timer)
{
	SyncScoreboard();
...
stock SavePlayerScores()
{
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i))
		{
			g_saved_kills[i] = GetEntProp(i, Prop_Data, "m_iFrags");
			g_saved_deaths[i] = GetEntProp(i, Prop_Data, "m_iDeaths");
		}
		else
		{
			g_saved_kills[i] = 0;
	...
	g_should_restore_player_scores = true;
}

stock RestorePlayerScores()
{
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientInGame(i) && !IsFakeClient(i))
		{
			SetEntProp(i, Prop_Data, "m_iFrags", g_saved_kills[i]);
			SetEntProp(i, Prop_Data, "m_iDeaths", g_saved_deaths[i]);
		}
	}
}

/* ========================================================================= */
/* Delayed End Match (3 second delay + server info from cfg) */
/* ========================================================================= */
...
public Action:Timer_ResetMatchAfterEnd(Handle:timer)
{
	ResetMatch(true);
}
...
DisplayEndMatchInfo()
{
	...
	BuildPath(Path_SM, path, sizeof(path), "../../cfg/warmod/on_match_end.cfg");
	...
	if (hFile != INVALID_HANDLE)
	{
		...
		{
			TrimString(line);
			if (strncmp(line, "// endinfo ", 11, false) == 0)
			{
...
public Action:Hook_RadioText(UserMsg:msg_id, Handle:bf, const players[], playersNum, bool:reliable, bool:init)
{
    ...
    new msg_dest = BfReadByte(bf);
    new client = BfReadByte(bf);
    
    decl String:msg_name[128];
    BfReadString(bf, msg_name, sizeof(msg_name));
    
    if (StrEqual(msg_name, "#Game_radio_location", false))
    {
        decl String:player_name[64];
        decl String:location[64];
        decl String:radio_msg[128];
        
        BfReadString(bf, player_name, sizeof(player_name));
        BfReadString(bf, location, sizeof(location)); // Discard location
        BfReadString(bf, radio_msg, sizeof(radio_msg));
        
        new Handle:dp;
        CreateDataTimer(0.0, Timer_SendRadioClean, dp, TIMER_FLAG_NO_MAPCHANGE);
        WritePackCell(dp, msg_dest);
        WritePackCell(dp, client);
        ...
        WritePackString(dp, player_name);
        WritePackString(dp, radio_msg);
...
public Action:Timer_SendRadioClean(Handle:timer, Handle:dp)
{
    ResetPack(dp);
    new msg_dest = ReadPackCell(dp);
    new client = ReadPackCell(dp);
    ...
    decl String:player_name[64];
    decl String:radio_msg[128];
    ReadPackString(dp, player_name, sizeof(player_name));
    ReadPackString(dp, radio_msg, sizeof(radio_msg));
    
    if (count > 0)
    {
        new Handle:new_bf = StartMessage("RadioText", targets, count, USERMSG_RELIABLE|USERMSG_BLOCKHOOKS);
        if (new_bf != INVALID_HANDLE)
        {
            BfWriteByte(new_bf, msg_dest);
            BfWriteByte(new_bf, client);
            BfWriteString(new_bf, "#Game_radio");
            BfWriteString(new_bf, player_name);
            BfWriteString(new_bf, radio_msg);
            EndMessage();
        }
    }
